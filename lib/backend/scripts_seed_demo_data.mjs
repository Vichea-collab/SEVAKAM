import admin from "firebase-admin";
import { auth, db } from "./src/config/firebase.js";

const CATEGORY_SEEDS = [
  { id: "seed_cat_cleaner", name: "Cleaner", icon: "cleaner" },
  { id: "seed_cat_plumber", name: "Plumber", icon: "plumber" },
  { id: "seed_cat_electrician", name: "Electrician", icon: "electrician" },
  { id: "seed_cat_appliance", name: "Home Appliance", icon: "appliance" },
  { id: "seed_cat_maintenance", name: "Home Maintenance", icon: "maintenance" },
];

const SERVICE_SEEDS = [
  {
    id: "seed_srv_pipe_repair",
    name: "Pipe leaks",
    categoryId: "seed_cat_plumber",
    categoryName: "Plumber",
    completedCount: 89,
    rating: 4.8,
    image: "assets/images/plumber/pipe-leak.jpg",
  },
  {
    id: "seed_srv_toilet_repair",
    name: "Toilet Repair",
    categoryId: "seed_cat_plumber",
    categoryName: "Plumber",
    completedCount: 51,
    rating: 4.6,
    image: "assets/images/plumber/toilet-repair.jpg",
  },
  {
    id: "seed_srv_water_installation",
    name: "Water Installation",
    categoryId: "seed_cat_plumber",
    categoryName: "Plumber",
    completedCount: 43,
    rating: 4.5,
    image: "assets/images/plumber/water-installation.jpg",
  },
  {
    id: "seed_srv_wiring_repair",
    name: "Wiring Repair",
    categoryId: "seed_cat_electrician",
    categoryName: "Electrician",
    completedCount: 93,
    rating: 4.8,
    image: "assets/images/electrician/wiring-repair.jpg",
  },
  {
    id: "seed_srv_light_fan_installation",
    name: "Light / Fan Installation",
    categoryId: "seed_cat_electrician",
    categoryName: "Electrician",
    completedCount: 47,
    rating: 4.5,
    image: "assets/images/electrician/fan-installation.jpg",
  },
  {
    id: "seed_srv_power_outage_fixes",
    name: "Power Outage Fixes",
    categoryId: "seed_cat_electrician",
    categoryName: "Electrician",
    completedCount: 40,
    rating: 4.5,
    image: "assets/images/electrician/power-outages-fix.jpg",
  },
  {
    id: "seed_srv_house_cleaning",
    name: "House Cleaning",
    categoryId: "seed_cat_cleaner",
    categoryName: "Cleaner",
    completedCount: 132,
    rating: 4.9,
    image: "assets/images/cleaning/house-cleaning.jpg",
  },
  {
    id: "seed_srv_office_cleaning",
    name: "Office Cleaning",
    categoryId: "seed_cat_cleaner",
    categoryName: "Cleaner",
    completedCount: 72,
    rating: 4.7,
    image: "assets/images/cleaning/office-cleaning.jpg",
  },
  {
    id: "seed_srv_move_in_out_cleaning",
    name: "Move-in / Move-out Cleaning",
    categoryId: "seed_cat_cleaner",
    categoryName: "Cleaner",
    completedCount: 55,
    rating: 4.7,
    image: "assets/images/cleaning/move-in-out-cleaning.jpg",
  },
  {
    id: "seed_srv_ac_repair",
    name: "Air Conditioner Repair",
    categoryId: "seed_cat_appliance",
    categoryName: "Home Appliance",
    completedCount: 67,
    rating: 4.8,
    image: "assets/images/home_appliance_repair/ac-repair.jpg",
  },
  {
    id: "seed_srv_washing_machine_repair",
    name: "Washing Machine Repair",
    categoryId: "seed_cat_appliance",
    categoryName: "Home Appliance",
    completedCount: 44,
    rating: 4.6,
    image: "assets/images/home_appliance_repair/washing-machine-repair.jpg",
  },
  {
    id: "seed_srv_refrigerator_repair",
    name: "Refrigerator Repair",
    categoryId: "seed_cat_appliance",
    categoryName: "Home Appliance",
    completedCount: 38,
    rating: 4.6,
    image: "assets/images/home_appliance_repair/refrigerator-repair.jpg",
  },
  {
    id: "seed_srv_door_repair",
    name: "Door & Window Repair",
    categoryId: "seed_cat_maintenance",
    categoryName: "Home Maintenance",
    completedCount: 54,
    rating: 4.6,
    image: "assets/images/home_maintenance/doorwindow-repair.jpg",
  },
  {
    id: "seed_srv_furniture_fixing",
    name: "Furniture Fixing",
    categoryId: "seed_cat_maintenance",
    categoryName: "Home Maintenance",
    completedCount: 46,
    rating: 4.6,
    image: "assets/images/home_maintenance/furniture-repair.jpg",
  },
  {
    id: "seed_srv_shelf_curtain_installation",
    name: "Shelf / Curtain Installation",
    categoryId: "seed_cat_maintenance",
    categoryName: "Home Maintenance",
    completedCount: 35,
    rating: 4.5,
    image: "assets/images/home_maintenance/shelf-curtain-installation.jpg",
  },
];

const PROMOTION_SEEDS = [
  {
    id: "seed_promo_house_cleaning",
    placement: "finder_home",
    badgeLabel: "Featured",
    title: "House Cleaning this week",
    description: "Book trusted house cleaning with faster response this week.",
    imageUrl: "assets/images/cleaning/house-cleaning.jpg",
    ctaLabel: "Book cleaning",
    targetType: "service",
    targetValue: "House Cleaning",
    query: "House Cleaning",
    category: "Cleaner",
    city: "Phnom Penh",
    sortOrder: 1,
    active: true,
  },
  {
    id: "seed_promo_pipe_leaks",
    placement: "finder_home",
    badgeLabel: "Fast response",
    title: "Pipe leaks fixed today",
    description: "Find plumbers near you for urgent pipe leaks and water issues.",
    imageUrl: "assets/images/plumber/pipe-leak.jpg",
    ctaLabel: "Find plumber",
    targetType: "service",
    targetValue: "Pipe leaks",
    query: "Pipe leaks",
    category: "Plumber",
    city: "Phnom Penh",
    sortOrder: 2,
    active: true,
  },
  {
    id: "seed_promo_power_outage",
    placement: "finder_home",
    badgeLabel: "Popular now",
    title: "Power Outage Fixes",
    description: "Restore power safely with electricians available now.",
    imageUrl: "assets/images/electrician/power-outages-fix.jpg",
    ctaLabel: "View electricians",
    targetType: "service",
    targetValue: "Power Outage Fixes",
    query: "Power Outage Fixes",
    category: "Electrician",
    city: "",
    sortOrder: 3,
    active: true,
  },
  {
    id: "seed_promo_ac_repair",
    placement: "finder_home",
    badgeLabel: "Home comfort",
    title: "Air Conditioner Repair",
    description: "Keep your home cool with quick repair from trusted appliance providers.",
    imageUrl: "assets/images/home_appliance_repair/ac-repair.jpg",
    ctaLabel: "Explore repair",
    targetType: "service",
    targetValue: "Air Conditioner Repair",
    query: "Air Conditioner Repair",
    category: "Home Appliance",
    city: "",
    sortOrder: 4,
    active: true,
  },
  {
    id: "seed_promo_furniture_fixing",
    placement: "finder_home",
    badgeLabel: "Weekend ready",
    title: "Furniture Fixing made easy",
    description: "Quick home maintenance for shelves, doors, and furniture jobs.",
    imageUrl: "assets/images/home_maintenance/furniture-repair.jpg",
    ctaLabel: "See options",
    targetType: "service",
    targetValue: "Furniture Fixing",
    query: "Furniture Fixing",
    category: "Home Maintenance",
    city: "",
    sortOrder: 5,
    active: true,
  },
];

const FINDER_SEEDS = [
  {
    key: "finder_a",
    email: "finder.demo1@sevakam.app",
    name: "Sok Dara",
    phoneNumber: "+85512345001",
    city: "Phnom Penh",
    location: "Toul Kork, Phnom Penh",
  },
  {
    key: "finder_b",
    email: "finder.demo2@sevakam.app",
    name: "Kim Sreypov",
    phoneNumber: "+85512345002",
    city: "Siem Reap",
    location: "Svay Dangkum, Siem Reap",
  },
  {
    key: "finder_c",
    email: "finder.demo3@sevakam.app",
    name: "Chan Vibol",
    phoneNumber: "+85512345003",
    city: "Battambang",
    location: "Battambang City",
  },
];

const PROVIDER_SEEDS = [
  {
    key: "provider_a",
    email: "provider.demo1@sevakam.app",
    name: "Nuth Chenda",
    phoneNumber: "+85517666001",
    city: "Phnom Penh",
    location: "Sen Sok, Phnom Penh",
    bio: "Residential and office cleaning with same-day service.",
    serviceName: "Cleaner",
    services: [
      "House Cleaning",
      "Office Cleaning",
      "Move-in / Move-out Cleaning",
    ],
    expertIn: "Deep cleaning, office cleaning",
    availableFrom: "08:00",
    availableTo: "20:00",
    experienceYears: "5",
    serviceArea: "Phnom Penh",
    providerType: "individual",
    companyName: "",
    maxWorkers: 1,
    subscriptionTier: "professional",
  },
  {
    key: "provider_cleaner_2",
    email: "provider.cleaner2@sevakam.app",
    name: "Srey Leak",
    phoneNumber: "+85517666011",
    city: "Phnom Penh",
    location: "Chbar Ampov, Phnom Penh",
    bio: "Eco-friendly cleaning services for homes and small offices.",
    serviceName: "Cleaner",
    services: ["House Cleaning", "Move-in / Move-out Cleaning"],
    expertIn: "House cleaning, move-in cleaning",
    availableFrom: "07:00",
    availableTo: "18:00",
    experienceYears: "3",
    serviceArea: "Phnom Penh",
    providerType: "individual",
    companyName: "",
    maxWorkers: 1,
  },
  {
    key: "provider_b",
    email: "provider.demo2@sevakam.app",
    name: "Seang Rith",
    phoneNumber: "+85517666002",
    city: "Phnom Penh",
    location: "Russey Keo, Phnom Penh",
    bio: "Plumbing and emergency leak repair specialist.",
    serviceName: "Plumber",
    services: ["Pipe leaks", "Toilet Repair", "Water Installation"],
    expertIn: "Pipe leak repair, drain cleaning",
    availableFrom: "09:00",
    availableTo: "21:00",
    experienceYears: "7",
    serviceArea: "Phnom Penh",
    providerType: "individual",
    companyName: "",
    maxWorkers: 1,
  },
  {
    key: "provider_plumber_2",
    email: "provider.plumber2@sevakam.app",
    name: "Heng Borin",
    phoneNumber: "+85517666022",
    city: "Phnom Penh",
    location: "Toul Kork, Phnom Penh",
    bio: "Expert plumber for industrial and residential water systems.",
    serviceName: "Plumber",
    services: ["Water Installation", "Toilet Repair", "Pipe leaks"],
    expertIn: "Water installation, toilet repair",
    availableFrom: "08:30",
    availableTo: "19:30",
    experienceYears: "10",
    serviceArea: "Phnom Penh",
    providerType: "individual",
    companyName: "",
    maxWorkers: 1,
  },
  {
    key: "provider_c",
    email: "provider.demo3@sevakam.app",
    name: "Vichea Maintenance Team",
    phoneNumber: "+85517666003",
    city: "Phnom Penh",
    location: "Chamkar Mon, Phnom Penh",
    bio: "Home maintenance team for electrical and appliance jobs.",
    serviceName: "Home Maintenance",
    services: [
      "Door & Window Repair",
      "Furniture Fixing",
      "Shelf / Curtain Installation",
    ],
    expertIn: "Wiring repair, AC repair, door/window fixes",
    availableFrom: "07:30",
    availableTo: "22:00",
    experienceYears: "9",
    serviceArea: "Phnom Penh & Kandal",
    providerType: "company",
    companyName: "Vichea Fix Co., Ltd.",
    maxWorkers: 4,
    subscriptionTier: "elite",
  },
  {
    key: "provider_maintenance_2",
    email: "provider.maint2@sevakam.app",
    name: "Samnang Pro Service",
    phoneNumber: "+85517666033",
    city: "Phnom Penh",
    location: "Dangkao, Phnom Penh",
    bio: "Professional home maintenance and furniture repair experts.",
    serviceName: "Home Maintenance",
    services: ["Furniture Fixing", "Shelf / Curtain Installation"],
    expertIn: "Furniture fixing, shelf installation",
    availableFrom: "08:00",
    availableTo: "18:00",
    experienceYears: "6",
    serviceArea: "Phnom Penh",
    providerType: "company",
    companyName: "Samnang Maintenance",
    maxWorkers: 3,
  },
  {
    key: "provider_electrician_1",
    email: "provider.elec1@sevakam.app",
    name: "Keo Sophea",
    phoneNumber: "+85517666044",
    city: "Phnom Penh",
    location: "Prampi Makara, Phnom Penh",
    bio: "Certified electrician for all household electrical needs.",
    serviceName: "Electrician",
    services: [
      "Wiring Repair",
      "Light / Fan Installation",
      "Power Outage Fixes",
    ],
    expertIn: "Wiring repair, light installation",
    availableFrom: "08:00",
    availableTo: "19:00",
    experienceYears: "5",
    serviceArea: "Phnom Penh",
    providerType: "individual",
    companyName: "",
    maxWorkers: 1,
  },
  {
    key: "provider_electrician_2",
    email: "provider.elec2@sevakam.app",
    name: "Long Bunthoeun",
    phoneNumber: "+85517666045",
    city: "Phnom Penh",
    location: "Boeng Keng Kang, Phnom Penh",
    bio: "Specialist in power outage fixes and complex wiring systems.",
    serviceName: "Electrician",
    services: ["Power Outage Fixes", "Wiring Repair"],
    expertIn: "Power outage fixes, wiring repair",
    availableFrom: "24/7",
    availableTo: "24/7",
    experienceYears: "12",
    serviceArea: "Phnom Penh",
    providerType: "individual",
    companyName: "",
    maxWorkers: 1,
    subscriptionTier: "professional",
  },
  {
    key: "provider_appliance_1",
    email: "provider.app1@sevakam.app",
    name: "Meng Hor",
    phoneNumber: "+85517666055",
    city: "Phnom Penh",
    location: "Meanchey, Phnom Penh",
    bio: "Home appliance repair for washing machines and fridges.",
    serviceName: "Home Appliance",
    services: [
      "Washing Machine Repair",
      "Refrigerator Repair",
      "Air Conditioner Repair",
    ],
    expertIn: "Washing machine repair, refrigerator repair",
    availableFrom: "09:00",
    availableTo: "18:00",
    experienceYears: "4",
    serviceArea: "Phnom Penh",
    providerType: "individual",
    companyName: "",
    maxWorkers: 1,
  },
  {
    key: "provider_appliance_2",
    email: "provider.app2@sevakam.app",
    name: "Tech Fix Cambodia",
    phoneNumber: "+85517666056",
    city: "Phnom Penh",
    location: "Toul Sangke, Phnom Penh",
    bio: "Fast and reliable appliance repairs including AC and dryers.",
    serviceName: "Home Appliance",
    services: [
      "Air Conditioner Repair",
      "Refrigerator Repair",
      "Washing Machine Repair",
    ],
    expertIn: "Air conditioner repair, gas refill",
    availableFrom: "08:00",
    availableTo: "20:00",
    experienceYears: "8",
    serviceArea: "Phnom Penh",
    providerType: "company",
    companyName: "Tech Fix Co.",
    maxWorkers: 5,
    subscriptionTier: "elite",
  },
];

const FINDER_POST_SEEDS = [
  {
    id: "seed_finder_post_1",
    finderKey: "finder_a",
    category: "Plumber",
    service: "Pipe Leak Repair",
    location: "Toul Kork, Phnom Penh",
    message: "Kitchen sink pipe is leaking and needs urgent repair tonight.",
    preferredDateOffsetDays: 1,
    createdHoursAgo: 2,
  },
  {
    id: "seed_finder_post_2",
    finderKey: "finder_b",
    category: "Cleaner",
    service: "Deep Cleaning",
    location: "Svay Dangkum, Siem Reap",
    message: "Need deep cleaning for a two-bedroom apartment this weekend.",
    preferredDateOffsetDays: 2,
    createdHoursAgo: 5,
  },
  {
    id: "seed_finder_post_3",
    finderKey: "finder_c",
    category: "Home Appliance",
    service: "Air Conditioner Repair",
    location: "Battambang City",
    message: "AC is not cooling and making noise. Looking for inspection.",
    preferredDateOffsetDays: 1,
    createdHoursAgo: 9,
  },
  {
    id: "seed_finder_post_4",
    finderKey: "finder_a",
    category: "Electrician",
    service: "Wiring Repair",
    location: "Boeng Keng Kang, Phnom Penh",
    message: "Power outlet sparks sometimes. Need safe rewiring.",
    preferredDateOffsetDays: 3,
    createdHoursAgo: 14,
  },
];

const PROVIDER_POST_SEEDS = [
  {
    id: "seed_provider_post_1",
    providerKey: "provider_a",
    category: "Cleaner",
    service: "House Cleaning",
    services: [
      "House Cleaning",
      "Office Cleaning",
      "Move-in / Move-out Cleaning",
    ],
    area: "Phnom Penh",
    details:
      "Supplies included, same-day support for condo and villa cleaning across house, office, and move-in jobs.",
    availableNow: true,
    createdHoursAgo: 1,
  },
  {
    id: "seed_provider_post_cleaner_2",
    providerKey: "provider_cleaner_2",
    category: "Cleaner",
    service: "House Cleaning",
    services: ["House Cleaning", "Move-in / Move-out Cleaning"],
    area: "Phnom Penh",
    details: "Thorough house cleaning with attention to detail. Eco-friendly products used.",
    availableNow: true,
    createdHoursAgo: 2,
  },
  {
    id: "seed_provider_post_3",
    providerKey: "provider_b",
    category: "Plumber",
    service: "Pipe leaks",
    services: ["Pipe leaks", "Toilet Repair", "Water Installation"],
    area: "Phnom Penh",
    details:
      "Emergency plumbing with spare parts for pipe leaks, toilet repair, and water installation jobs.",
    availableNow: true,
    createdHoursAgo: 3,
  },
  {
    id: "seed_provider_post_plumber_2",
    providerKey: "provider_plumber_2",
    category: "Plumber",
    service: "Water Installation",
    services: ["Water Installation", "Toilet Repair", "Pipe leaks"],
    area: "Phnom Penh",
    details: "Professional water system installation and toilet repair services.",
    availableNow: true,
    createdHoursAgo: 4,
  },
  {
    id: "seed_provider_post_5",
    providerKey: "provider_maintenance_2",
    category: "Home Maintenance",
    service: "Furniture Fixing",
    services: [
      "Furniture Fixing",
      "Door & Window Repair",
      "Shelf / Curtain Installation",
    ],
    area: "Phnom Penh",
    details:
      "Team-based home maintenance for furniture repair, door and window fixes, and shelf installation.",
    availableNow: true,
    createdHoursAgo: 4,
  },
  {
    id: "seed_provider_post_electrician_1",
    providerKey: "provider_electrician_1",
    category: "Electrician",
    service: "Wiring Repair",
    services: [
      "Wiring Repair",
      "Light / Fan Installation",
      "Power Outage Fixes",
    ],
    area: "Phnom Penh",
    details:
      "Reliable electrical work covering rewiring, light and fan installation, and urgent power outage fixes.",
    availableNow: true,
    createdHoursAgo: 5,
  },
  {
    id: "seed_provider_post_electrician_2",
    providerKey: "provider_electrician_2",
    category: "Electrician",
    service: "Power Outage Fixes",
    services: ["Power Outage Fixes", "Wiring Repair"],
    area: "Phnom Penh",
    details: "Quick response for electrical power outages and urgent repairs.",
    availableNow: true,
    createdHoursAgo: 1,
  },
  {
    id: "seed_provider_post_6",
    providerKey: "provider_appliance_2",
    category: "Home Appliance",
    service: "Air Conditioner Repair",
    services: [
      "Air Conditioner Repair",
      "Refrigerator Repair",
      "Washing Machine Repair",
    ],
    area: "Phnom Penh",
    details:
      "Diagnosis, gas refill, and repair support for AC, refrigerator, and washing machine jobs.",
    availableNow: true,
    createdHoursAgo: 7,
  },
  {
    id: "seed_provider_post_appliance_1",
    providerKey: "provider_appliance_1",
    category: "Home Appliance",
    service: "Washing Machine Repair",
    services: [
      "Washing Machine Repair",
      "Refrigerator Repair",
      "Air Conditioner Repair",
    ],
    area: "Phnom Penh",
    details: "Fixing all major brands of washing machines and refrigerators.",
    availableNow: true,
    createdHoursAgo: 2,
  },
  {
    id: "seed_provider_post_maintenance_2",
    providerKey: "provider_c",
    category: "Home Maintenance",
    service: "Door & Window Repair",
    services: [
      "Door & Window Repair",
      "Furniture Fixing",
      "Shelf / Curtain Installation",
    ],
    area: "Phnom Penh",
    details:
      "Company support for door and window repair, furniture fixing, and shelf or curtain installation.",
    availableNow: true,
    createdHoursAgo: 6,
  },
];

const ORDER_REVIEW_SEEDS = [
  {
    id: "seed_order_review_1",
    finderKey: "finder_a",
    providerKey: "provider_b",
    categoryName: "Plumber",
    serviceName: "Pipe Leak Repair",
    addressLabel: "Home",
    addressStreet: "Street 271, Toul Kork",
    addressCity: "Phnom Penh",
    addressMapLink: "https://maps.google.com/?q=Street+271+Toul+Kork+Phnom+Penh",
    preferredTimeSlot: "09:00 - 11:00",
    homeType: "apartment",
    completedDaysAgo: 6,
    rating: 5.0,
    comment: "Very professional and fixed the leak quickly.",
  },
  {
    id: "seed_order_review_2",
    finderKey: "finder_b",
    providerKey: "provider_b",
    categoryName: "Plumber",
    serviceName: "Drain Cleaning",
    addressLabel: "Office",
    addressStreet: "Sangkat Boeng Keng Kang, Phnom Penh",
    addressCity: "Phnom Penh",
    addressMapLink: "https://maps.google.com/?q=Boeng+Keng+Kang+Phnom+Penh",
    preferredTimeSlot: "13:00 - 15:00",
    homeType: "office",
    completedDaysAgo: 12,
    rating: 4.5,
    comment: "Good service and clear communication.",
  },
];

function parseArgs(argv) {
  const args = {
    project: "",
    password: "Demo@123456",
    execute: false,
  };

  for (let index = 0; index < argv.length; index += 1) {
    const token = argv[index];
    if (token === "--project") {
      args.project = (argv[index + 1] || "").trim();
      index += 1;
      continue;
    }
    if (token === "--password") {
      args.password = (argv[index + 1] || "").trim() || args.password;
      index += 1;
      continue;
    }
    if (token === "--execute") {
      args.execute = true;
      continue;
    }
    if (token === "--help" || token === "-h") {
      printUsage();
      process.exit(0);
    }
  }

  return args;
}

function printUsage() {
  console.log("Usage:");
  console.log(
    "  node scripts_seed_demo_data.mjs --project <project-id> --password <password> --execute",
  );
  console.log("");
  console.log("Options:");
  console.log("  --project   Optional safety check for Firebase project id.");
  console.log("  --password  Password for demo auth users. Default: Demo@123456");
  console.log("  --execute   Required to write data.");
}

function toTimestampFromHoursAgo(hoursAgo) {
  const date = new Date(Date.now() - Math.max(0, hoursAgo) * 60 * 60 * 1000);
  return admin.firestore.Timestamp.fromDate(date);
}

function toIsoFromDaysAhead(daysAhead) {
  const date = new Date(Date.now() + Math.max(0, daysAhead) * 24 * 60 * 60 * 1000);
  return date.toISOString();
}

function toTimestampFromDaysAgo(daysAgo) {
  const date = new Date(Date.now() - Math.max(0, daysAgo) * 24 * 60 * 60 * 1000);
  return admin.firestore.Timestamp.fromDate(date);
}

async function ensureAuthUser({ email, displayName, password, role }) {
  const claims = { role, roles: [role] };
  try {
    const existing = await auth.getUserByEmail(email);
    await auth.updateUser(existing.uid, {
      displayName,
      password,
      emailVerified: true,
      disabled: false,
    });
    await auth.setCustomUserClaims(existing.uid, claims);
    return { uid: existing.uid, created: false };
  } catch (error) {
    if (error?.code !== "auth/user-not-found") throw error;
    const created = await auth.createUser({
      email,
      password,
      displayName,
      emailVerified: true,
      disabled: false,
    });
    await auth.setCustomUserClaims(created.uid, claims);
    return { uid: created.uid, created: true };
  }
}

async function upsertUserDoc(uid, profile, role) {
  const ref = db.collection("users").doc(uid);
  const snap = await ref.get();
  const existing = snap.exists ? snap.data() || {} : {};

  const roleSet = new Set();
  if (existing.role) roleSet.add(existing.role.toString().toLowerCase());
  if (Array.isArray(existing.roles)) {
    existing.roles.forEach((value) => roleSet.add((value || "").toString().toLowerCase()));
  }
  roleSet.add(role);
  const roles = Array.from(roleSet).filter((value) => value === "finder" || value === "provider");

  const now = admin.firestore.FieldValue.serverTimestamp();
  const payload = {
    name: profile.name,
    email: profile.email,
    role,
    roles,
    photoUrl: "",
    updatedAt: now,
  };
  if (!snap.exists) {
    payload.createdAt = now;
  }

  await ref.set(payload, { merge: true });
}

async function upsertFinderProfile(uid, seed) {
  const ref = db.collection("finders").doc(uid);
  const snap = await ref.get();
  const now = admin.firestore.FieldValue.serverTimestamp();
  const payload = {
    city: seed.city,
    location: seed.location,
    phoneNumber: seed.phoneNumber,
    birthday: "1998-01-12",
    PhotoUrl: "",
    updatedAt: now,
  };
  if (!snap.exists) {
    payload.createdAt = now;
  }
  await ref.set(payload, { merge: true });
}

async function upsertProviderProfile(uid, seed) {
  const ref = db.collection("providers").doc(uid);
  const snap = await ref.get();
  const now = admin.firestore.FieldValue.serverTimestamp();
  const payload = {
    city: seed.city,
    location: seed.location,
    phoneNumber: seed.phoneNumber,
    birthday: "1995-05-20",
    bio: seed.bio,
    PhotoUrl: "",
    serviceName: seed.serviceName,
    services: seed.services || [],
    serviceId: "",
    serviceImageUrl: "",
    expertIn: seed.expertIn,
    availableFrom: seed.availableFrom,
    availableTo: seed.availableTo,
    experienceYears: seed.experienceYears,
    serviceArea: seed.serviceArea,
    providerType: seed.providerType,
    companyName: seed.providerType === "company" ? seed.companyName : "",
    maxWorkers: seed.providerType === "company" ? seed.maxWorkers : 1,
    ratingCount: 0,
    ratingSum: 0,
    activeOrder: 0,
    completedOrder: 0,
    subscription: {
      tier: seed.subscriptionTier || "basic",
      status: "active",
      stripeCustomerId: "",
      stripeSubscriptionId: "",
      currentPeriodStart: null,
      currentPeriodEnd: null,
      cancelAtPeriodEnd: false,
    },
    bookingsThisPeriod: 0,
    updatedAt: now,
  };
  if (!snap.exists) {
    payload.createdAt = now;
  }
  await ref.set(payload, { merge: true });
}

async function seedCategoriesAndServices() {
  const now = admin.firestore.FieldValue.serverTimestamp();
  const batch = db.batch();

  CATEGORY_SEEDS.forEach((category) => {
    const ref = db.collection("categories").doc(category.id);
    batch.set(
      ref,
      {
        id: category.id,
        name: category.name,
        icon: category.icon,
        isActive: true,
        updatedAt: now,
        createdAt: now,
      },
      { merge: true },
    );
  });

  SERVICE_SEEDS.forEach((service) => {
    const ref = db.collection("services").doc(service.id);
    batch.set(
      ref,
      {
        id: service.id,
        name: service.name,
        categoryId: service.categoryId,
        categoryName: service.categoryName,
        active: true,
        available: true,
        completedCount: service.completedCount,
        rating: service.rating,
        image: service.image || "",
        updatedAt: now,
        createdAt: now,
      },
      { merge: true },
    );
  });

  await batch.commit();
  await cleanupStaleSeedServices();
}

async function cleanupStaleSeedServices() {
  const keepIds = new Set(SERVICE_SEEDS.map((item) => item.id));
  const staleSnap = await db
    .collection("services")
    .where(
      admin.firestore.FieldPath.documentId(),
      ">=",
      "seed_srv_",
    )
    .where(
      admin.firestore.FieldPath.documentId(),
      "<",
      "seed_srv_\uf8ff",
    )
    .get();
  if (staleSnap.empty) return;
  const batch = db.batch();
  let deleteCount = 0;
  staleSnap.docs.forEach((doc) => {
    if (keepIds.has(doc.id)) return;
    batch.delete(doc.ref);
    deleteCount += 1;
  });
  if (deleteCount > 0) {
    await batch.commit();
    console.log(`Removed stale seeded services: ${deleteCount}`);
  }
}

async function seedFinderPosts(finderUidByKey, finderByKey) {
  const batch = db.batch();

  FINDER_POST_SEEDS.forEach((post) => {
    const finderUid = finderUidByKey.get(post.finderKey) || "";
    const finder = finderByKey.get(post.finderKey);
    if (!finderUid || !finder) return;
    const createdAt = toTimestampFromHoursAgo(post.createdHoursAgo);
    const ref = db.collection("finderPosts").doc(post.id);
    batch.set(
      ref,
      {
        id: post.id,
        finderUid,
        clientName: finder.name,
        clientAvatarUrl: "",
        category: post.category,
        service: post.service,
        location: post.location,
        message: post.message,
        preferredDate: toIsoFromDaysAhead(post.preferredDateOffsetDays),
        status: "open",
        createdAt,
        updatedAt: createdAt,
      },
      { merge: true },
    );
  });

  await batch.commit();
}

async function seedProviderPosts(providerUidByKey, providerByKey) {
  const batch = db.batch();

  PROVIDER_POST_SEEDS.forEach((post) => {
    const providerUid = providerUidByKey.get(post.providerKey) || "";
    const provider = providerByKey.get(post.providerKey);
    if (!providerUid || !provider) return;
    const createdAt = toTimestampFromHoursAgo(post.createdHoursAgo);
    const ref = db.collection("providerPosts").doc(post.id);
    batch.set(
      ref,
      {
        id: post.id,
        providerUid,
        providerName: provider.name,
        providerAvatarUrl: "",
        category: post.category,
        service: post.service,
        services: Array.isArray(post.services) ? post.services : [post.service],
        area: post.area,
        details: post.details,
        availableNow: post.availableNow === true,
        providerType: provider.providerType,
        providerCompanyName: provider.providerType === "company" ? provider.companyName : "",
        providerMaxWorkers: provider.providerType === "company" ? provider.maxWorkers : 1,
        status: "open",
        createdAt,
        updatedAt: createdAt,
      },
      { merge: true },
    );
  });

  await batch.commit();
}

async function seedCompletedReviewOrders(
  finderUidByKey,
  providerUidByKey,
  finderByKey,
  providerByKey,
) {
  const batch = db.batch();
  const providerStats = new Map();

  ORDER_REVIEW_SEEDS.forEach((seed) => {
    const finderUid = finderUidByKey.get(seed.finderKey) || "";
    const providerUid = providerUidByKey.get(seed.providerKey) || "";
    const finder = finderByKey.get(seed.finderKey);
    const provider = providerByKey.get(seed.providerKey);
    if (!finderUid || !providerUid || !finder || !provider) return;

    const completedAt = toTimestampFromDaysAgo(seed.completedDaysAgo);
    const completedDate = completedAt.toDate();
    const startedAt = admin.firestore.Timestamp.fromDate(
      new Date(completedDate.getTime() - 90 * 60 * 1000),
    );
    const onTheWayAt = admin.firestore.Timestamp.fromDate(
      new Date(completedDate.getTime() - 150 * 60 * 1000),
    );
    const bookedAt = admin.firestore.Timestamp.fromDate(
      new Date(completedDate.getTime() - 5 * 60 * 60 * 1000),
    );
    const reviewedAt = admin.firestore.Timestamp.fromDate(
      new Date(completedDate.getTime() + 30 * 60 * 1000),
    );
    const preferredDate = new Date(completedDate.getTime() - 24 * 60 * 60 * 1000)
      .toISOString();

    const ref = db.collection("orders").doc(seed.id);
    batch.set(
      ref,
      {
        id: seed.id,
        finderUid,
        finderName: finder.name,
        finderPhone: finder.phoneNumber,
        providerUid,
        providerName: provider.name,
        providerRole: provider.serviceName,
        providerRating: Number(seed.rating.toFixed(1)),
        providerImagePath: "",
        providerType: provider.providerType,
        providerCompanyName: provider.providerType === "company" ? provider.companyName : "",
        providerMaxWorkers: provider.providerType === "company" ? provider.maxWorkers : 1,
        categoryName: seed.categoryName,
        serviceName: seed.serviceName,
        addressLabel: seed.addressLabel,
        addressStreet: seed.addressStreet,
        addressCity: seed.addressCity,
        addressMapLink: seed.addressMapLink,
        preferredDate,
        preferredTimeSlot: seed.preferredTimeSlot,
        homeType: seed.homeType,
        additionalService: "",
        finderNote: "",
        serviceFields: {},
        finderRating: seed.rating,
        finderComment: seed.comment,
        reviewedAt,
        status: "completed",
        statusTimeline: {
          bookedAt,
          onTheWayAt,
          startedAt,
          completedAt,
        },
        createdAt: bookedAt,
        updatedAt: reviewedAt,
      },
      { merge: true },
    );

    const current = providerStats.get(providerUid) || {
      ratingCount: 0,
      ratingSum: 0,
      completedOrder: 0,
    };
    current.ratingCount += 1;
    current.ratingSum += seed.rating;
    current.completedOrder += 1;
    providerStats.set(providerUid, current);
  });

  await batch.commit();
  if (providerStats.size == 0) return;

  const providerBatch = db.batch();
  providerStats.forEach((stats, providerUid) => {
    providerBatch.set(
      db.collection("providers").doc(providerUid),
      {
        ratingCount: stats.ratingCount,
        ratingSum: Number(stats.ratingSum.toFixed(1)),
        completedOrder: stats.completedOrder,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      },
      { merge: true },
    );
  });
  await providerBatch.commit();
}

async function seedPromotions() {
  const now = admin.firestore.FieldValue.serverTimestamp();
  const promotionCollection = db.collection("adminPromotions");
  const seededSnapshot = await promotionCollection
    .where("createdByUid", "==", "seed_script")
    .get();
  const validIds = new Set(PROMOTION_SEEDS.map((promotion) => promotion.id));
  const batch = db.batch();

  seededSnapshot.docs.forEach((doc) => {
    if (!validIds.has(doc.id)) {
      batch.delete(doc.ref);
    }
  });

  PROMOTION_SEEDS.forEach((promotion) => {
    const ref = promotionCollection.doc(promotion.id);
    batch.set(
      ref,
      {
        id: promotion.id,
        placement: promotion.placement,
        badgeLabel: promotion.badgeLabel,
        title: promotion.title,
        description: promotion.description,
        imageUrl: promotion.imageUrl,
        ctaLabel: promotion.ctaLabel,
        targetType: promotion.targetType,
        targetValue: promotion.targetValue,
        query: promotion.query,
        category: promotion.category,
        city: promotion.city,
        targetRoles: ["finder"],
        sortOrder: promotion.sortOrder,
        active: promotion.active !== false,
        startAt: null,
        endAt: null,
        createdByUid: "seed_script",
        createdByName: "Demo Seed",
        updatedAt: now,
        createdAt: now,
      },
      { merge: true },
    );
  });

  await batch.commit();
}

function validateProviderServiceCoverage() {
  const coveredServices = new Set();
  PROVIDER_POST_SEEDS.forEach((post) => {
    const services = Array.isArray(post.services) ? post.services : [post.service];
    services
      .map((service) => (service || "").toString().trim())
      .filter(Boolean)
      .forEach((service) => coveredServices.add(service));
  });

  const uncovered = SERVICE_SEEDS
    .map((service) => service.name)
    .filter((name) => !coveredServices.has(name));

  if (uncovered.length > 0) {
    throw new Error(
      `Seed data invalid. These services have no provider coverage: ${uncovered.join(", ")}`,
    );
  }
}

function assertProjectId(expectedProject) {
  const configuredProject = (auth.app.options.projectId || "").toString().trim();
  if (!expectedProject) {
    console.log(`Firebase project: ${configuredProject || "(unknown)"}`);
    return;
  }
  if (configuredProject && configuredProject !== expectedProject) {
    throw new Error(
      `Project mismatch. Expected "${expectedProject}", got "${configuredProject}".`,
    );
  }
  console.log(`Firebase project verified: ${expectedProject}`);
}

async function main() {
  const args = parseArgs(process.argv.slice(2));
  if (!args.execute) {
    printUsage();
    console.log("");
    console.log("Dry run only. Re-run with --execute to write seed data.");
    process.exit(1);
  }

  assertProjectId(args.project);
  validateProviderServiceCoverage();

  const finderUidByKey = new Map();
  const providerUidByKey = new Map();
  const finderByKey = new Map(FINDER_SEEDS.map((item) => [item.key, item]));
  const providerByKey = new Map(PROVIDER_SEEDS.map((item) => [item.key, item]));

  let createdAuthUsers = 0;
  let updatedAuthUsers = 0;

  for (const finder of FINDER_SEEDS) {
    const authUser = await ensureAuthUser({
      email: finder.email,
      displayName: finder.name,
      password: args.password,
      role: "finder",
    });
    if (authUser.created) createdAuthUsers += 1;
    else updatedAuthUsers += 1;
    finderUidByKey.set(finder.key, authUser.uid);
    await upsertUserDoc(authUser.uid, finder, "finder");
    await upsertFinderProfile(authUser.uid, finder);
  }

  for (const provider of PROVIDER_SEEDS) {
    const authUser = await ensureAuthUser({
      email: provider.email,
      displayName: provider.name,
      password: args.password,
      role: "provider",
    });
    if (authUser.created) createdAuthUsers += 1;
    else updatedAuthUsers += 1;
    providerUidByKey.set(provider.key, authUser.uid);
    await upsertUserDoc(authUser.uid, provider, "provider");
    await upsertProviderProfile(authUser.uid, provider);
  }

  await seedCategoriesAndServices();
  await seedFinderPosts(finderUidByKey, finderByKey);
  await seedProviderPosts(providerUidByKey, providerByKey);
  await seedCompletedReviewOrders(
    finderUidByKey,
    providerUidByKey,
    finderByKey,
    providerByKey,
  );
  await seedPromotions();

  console.log("Seed completed.");
  console.log(`Auth users created: ${createdAuthUsers}`);
  console.log(`Auth users updated: ${updatedAuthUsers}`);
  console.log(`Categories upserted: ${CATEGORY_SEEDS.length}`);
  console.log(`Services upserted: ${SERVICE_SEEDS.length}`);
  console.log(`Finder posts upserted: ${FINDER_POST_SEEDS.length}`);
  console.log(`Provider posts upserted: ${PROVIDER_POST_SEEDS.length}`);
  console.log(`Reviewed completed orders upserted: ${ORDER_REVIEW_SEEDS.length}`);
  console.log(`Promotions upserted: ${PROMOTION_SEEDS.length}`);
  console.log(`Demo password: ${args.password}`);
}

await main();
