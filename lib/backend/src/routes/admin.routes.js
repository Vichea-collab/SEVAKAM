import { Router } from "express";
import { requireAuth } from "../middlewares/auth.middleware.js";
import { requireRole } from "../middlewares/role.middleware.js";
import {
  validateAdminBroadcastActiveUpdate,
  validateAdminBroadcastCreate,
  validateAdminPromotionActiveUpdate,
  validateAdminPromotionCreate,
} from "../middlewares/validation.middleware.js";
import AdminController from "../controllers/admin.controller.js";

const router = Router();

router.get(
  "/access",
  requireAuth,
  requireRole(["admin"]),
  AdminController.verifyAccess,
);
router.get(
  "/overview",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getOverview,
);
router.get(
  "/read-budget",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getReadBudget,
);
router.get(
  "/search",
  requireAuth,
  requireRole(["admin"]),
  AdminController.globalSearch,
);
router.get(
  "/users",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getUsers,
);
router.patch(
  "/users/:id/status",
  requireAuth,
  requireRole(["admin"]),
  AdminController.updateUserStatus,
);
router.patch(
  "/providers/:id/kyc",
  requireAuth,
  requireRole(["admin"]),
  AdminController.updateProviderKycStatus,
);
router.get(
  "/orders",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getOrders,
);
router.patch(
  "/orders/:id/status",
  requireAuth,
  requireRole(["admin"]),
  AdminController.updateOrderStatus,
);
router.get(
  "/posts",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getPosts,
);
router.patch(
  "/posts/:source/:id/status",
  requireAuth,
  requireRole(["admin"]),
  AdminController.updatePostStatus,
);
router.get(
  "/tickets",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getTickets,
);
router.patch(
  "/tickets/:userUid/:id/status",
  requireAuth,
  requireRole(["admin"]),
  AdminController.updateTicketStatus,
);
router.get(
  "/tickets/:userUid/:id/messages",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getTicketMessages,
);
router.post(
  "/tickets/:userUid/:id/messages",
  requireAuth,
  requireRole(["admin"]),
  AdminController.sendTicketMessage,
);
router.get(
  "/promotions",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getPromotions,
);
router.post(
  "/promotions",
  requireAuth,
  requireRole(["admin"]),
  validateAdminPromotionCreate,
  AdminController.createPromotion,
);
router.patch(
  "/promotions/:id/active",
  requireAuth,
  requireRole(["admin"]),
  validateAdminPromotionActiveUpdate,
  AdminController.updatePromotionActive,
);
router.get(
  "/broadcasts",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getBroadcasts,
);
router.post(
  "/broadcasts",
  requireAuth,
  requireRole(["admin"]),
  validateAdminBroadcastCreate,
  AdminController.createBroadcast,
);
router.patch(
  "/broadcasts/:id/active",
  requireAuth,
  requireRole(["admin"]),
  validateAdminBroadcastActiveUpdate,
  AdminController.updateBroadcastActive,
);
router.get(
  "/services",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getServices,
);
router.patch(
  "/services/:id/active",
  requireAuth,
  requireRole(["admin"]),
  AdminController.updateServiceActive,
);
router.get(
  "/actions/history",
  requireAuth,
  requireRole(["admin"]),
  AdminController.getUndoHistory,
);
router.post(
  "/actions/undo",
  requireAuth,
  requireRole(["admin"]),
  AdminController.undoAction,
);

export default router;
