...
const MAX_ADMIN_PROMOTION_SCAN = 200;
...
const ALLOWED_KYC_STATUSES = ["unverified", "pending", "approved", "rejected"];
const ALLOWED_BROADCAST_TYPES = ["system", "promotion"];
const ALLOWED_PROMOTION_TARGET_TYPES = ["provider", "service", "category", "search", "post", "page"];
const ALLOWED_PROMOTION_PLACEMENTS = ["finder_home"];
const PLAN_PRICES = {
  basic: 0,
  professional: 5,
  elite: 10,
...
function subscriptionMonthlyPrice(row = {}) {
  const tier = safeString(row?.subscription?.tier).toLowerCase() || "basic";
  return Number(PLAN_PRICES[tier] || 0);
}

function countsTowardSubscriptionRevenue(row = {}) {
  const status = safeString(row?.subscription?.status).toLowerCase();
  return status === "active" || status === "trialing";
}

function isPendingTicketStatus(status) {
  return ["open", "waiting_on_admin", "waiting_on_user"].includes(normalizeStatus(status));
}
...
function normalizePromotionTargetType(value) {
  const type = normalizeStatus(value, "search");
  if (ALLOWED_PROMOTION_TARGET_TYPES.includes(type)) return type;
  return "search";
}

function normalizePromotionPlacement(value) {
  const placement = normalizeStatus(value, "finder_home");
  if (ALLOWED_PROMOTION_PLACEMENTS.includes(placement)) return placement;
  return "finder_home";
}
...
function providerKycStatus(row = {}) {
  const direct = normalizeStatus(row.kycStatus);
  if (ALLOWED_KYC_STATUSES.includes(direct)) return direct;
  if (row.verified === true) return "approved";
  return "unverified";
}

function assertReasonIfRequired(reason, required) {
  if (!required) return;
  if (safeString(reason).length >= 3) return;
  const error = new Error("reason is required (min 3 chars)");
  error.status = 400;
  throw error;
}

function adminUserStatusNeedsReason(active) {
  return active === false;
}

function adminProviderKycNeedsReason(status) {
  return ["rejected", "unverified"].includes(safeString(status).toLowerCase());
}

function adminOrderStatusNeedsReason(status) {
  return safeString(status).toLowerCase() === "cancelled";
}

function adminPostStatusNeedsReason(status) {
  return safeString(status).toLowerCase() === "closed";
}

function adminTicketStatusNeedsReason(status) {
  return safeString(status).toLowerCase() === "closed";
}

function adminServiceActiveNeedsReason(active) {
  return active === false;
}
...
class AdminService {
  static async verifyAccess(uid, user = {}) {
  ...
  static async _attachProviderAdminFields(items = []) {
    const providerItems = items.filter((item) => safeString(item.role).toLowerCase().includes("provider"));
    if (providerItems.length === 0) return items;

    const providerIds = Array.from(
      new Set(providerItems.map((item) => safeString(item.id)).filter(Boolean)),
    );
    const snaps = await Promise.all(
      providerIds.map((providerId) => db.collection("providers").doc(providerId).get()),
    );
    await this._recordReadUsage(snaps.length, { endpoint: "users_provider_meta" });
    const providerMap = new Map();
    snaps.forEach((snap) => {
      if (!snap.exists) return;
      providerMap.set(snap.id, snap.data() || {});
    });

    return items.map((item) => {
      const providerRow = providerMap.get(item.id) || {};
      const sub = providerRow.subscription || {};
      return {
        ...item,
        providerKycStatus: providerKycStatus(providerRow),
        providerVerified: providerRow.verified === true || providerKycStatus(providerRow) === "approved",
        providerSubscriptionTier: safeString(sub.tier) || "basic",
        providerSubscriptionStatus: safeString(sub.status) || "inactive",
        providerSubscriptionCancelAtPeriodEnd: sub.cancelAtPeriodEnd === true,
        providerSubscriptionPeriodEnd: toIso(sub.currentPeriodEnd),
        providerKycIdFrontUrl: safeString(providerRow.kycIdFrontUrl),
        providerKycIdBackUrl: safeString(providerRow.kycIdBackUrl),
        providerKycSubmittedAt: toIso(providerRow.kycSubmittedAt),
      };
    });
  }
  ...
  static async getOverview(uid, user = {}) {
    ...
    const openTickets = tickets.filter((item) => isPendingTicketStatus(item.status)).length;
    ...
    const providerDocs = providersSnap.docs.map((doc) => doc.data() || {});
    const pendingKycProviders = providerDocs.filter(
      (row) => providerKycStatus(row) === "pending",
    ).length;
    const verifiedProviders = providerDocs.filter(
      (row) => providerKycStatus(row) === "approved",
    ).length;
    const eliteProviders = providerDocs.filter(
      (row) => safeString(row?.subscription?.tier).toLowerCase() === "elite",
    ).length;
    const professionalProviders = providerDocs.filter(
      (row) => safeString(row?.subscription?.tier).toLowerCase() === "professional",
    ).length;
    const subscriptionMonthlyRevenue = providerDocs.reduce(
      (sum, row) => (
        countsTowardSubscriptionRevenue(row)
          ? sum + subscriptionMonthlyPrice(row)
          : sum
      ),
      0,
    );
    const totalRevenue = revenueCompleted + subscriptionMonthlyRevenue;
    ...
    const overview = {
      ...
      kpis: {
        ...
        subscriptionMonthlyRevenue: Number(subscriptionMonthlyRevenue.toFixed(2)),
  ...
  static async getUsers(uid, user, pagination, filters = {}) {
    ...
    if (canUseCursor) {
      ...
      const items = await this._attachProviderAdminFields(docs.map((doc) => {
        const row = doc.data() || {};
        return {
          id: doc.id,
          name: safeString(row.name) || "Unnamed User",
    ...
    await this._recordReadUsage(snap.size, { endpoint: "users" });
    const items = await this._attachProviderAdminFields(snap.docs.map((doc) => {
      const row = doc.data() || {};
      return {
        id: doc.id,
        name: safeString(row.name) || "Unnamed User",
  ...
  static async getTickets(uid, user, pagination, filters = {}) {
    ...
    const items = snap.docs.map((doc) => {
      ...
      return {
        ...
        subcategory: safeString(row.subcategory) || "other_issue",
        priority: safeString(row.priority) || "normal",
        status: safeString(row.status) || "open",
        createdAt: toIso(row.createdAt),
        lastMessageAt: toIso(row.lastMessageAt || row.updatedAt || row.createdAt),
        lastMessageSenderRole: safeString(row.lastMessageSenderRole),
        lastMessageSenderName: safeString(row.lastMessageSenderName),
      };
    });
    ...
    if (userUids.length > 0) {
      ...
      items.forEach((item) => {
        ...
        item.userRole = normalizeRole(userRow.role) || "finder";
      });
    ...
    const filtered = items.filter((row) => {
      ...
      if (categoryFilter && categoryFilter !== "all") {
        if (safeString(row.category).toLowerCase() !== categoryFilter) return false;
      }
      if (!query) return true;
      return mapRowContains(
        [
    ...
    const sorted = sortByNewest(
      filtered,
      (row) => toMillis(row.lastMessageAt) || toMillis(row.createdAt),
    );
    const paged = paginateArray(sorted, pagination);
    return { data: paged.items, pagination: paged.pagination };
  }
  ...
  static async sendTicketMessage(uid, user, userUid, ticketId, payload = {}) {
    ...
    const nextStatus = "waiting_on_user";
  ...
  static async getPromotions(uid, user, pagination, filters = {}) {
    await this._assertAdmin(uid, user);
    const placementFilter = normalizePromotionPlacement(
      safeString(filters.placement || "finder_home"),
    );
    const targetTypeFilter = safeString(filters.targetType).toLowerCase();
    const statusFilter = normalizeStatus(filters.status, "all");
    const query = safeString(filters.q || filters.query).toLowerCase();
    const normalized = normalizePagination(pagination, MAX_ADMIN_PROMOTION_SCAN);
    const canUseCursor =
      (!targetTypeFilter || targetTypeFilter === "all") &&
      (!statusFilter || statusFilter === "all") &&
      !query &&
      (normalized.page === 1 || Boolean(normalized.cursor));
    ...
    let snap = null;
    if (canUseCursor) {
      let queryRef = db.collection("adminPromotions").orderBy("createdAt", "desc");
      queryRef = await this._applyDocCursor(queryRef, "adminPromotions", normalized.cursor);
      const fetchLimit = normalized.cursor ? normalized.limit + 1 : normalized.limit;
      snap = await queryRef.limit(fetchLimit).get();
    } else {
      const queryLimit = readWindow(pagination, MAX_ADMIN_PROMOTION_SCAN);
      snap = await db.collection("adminPromotions").limit(queryLimit).get();
    }
    await this._recordReadUsage(snap.size, { endpoint: "admin_promotions" });
    const now = Date.now();
    ...
    const items = sourceDocs
      .map((doc) => {
        ...
        return {
          ...
          sortOrder: Math.max(0, Math.floor(toNumber(row.sortOrder, 0))),
          active,
          lifecycle: lifecycleForWindow({ active, startAt, endAt, now }),
          startAt,
          endAt,
          createdAt: toIso(row.createdAt),
          updatedAt: toIso(row.updatedAt),
      ...
      .filter((row) => {
        if (row.placement !== placementFilter) return false;
        if (targetTypeFilter && targetTypeFilter !== "all" && row.targetType !== targetTypeFilter) {
          return false;
        }
        if (statusFilter && statusFilter !== "all" && row.lifecycle !== statusFilter) {
          return false;
        }
        if (!query) return true;
        return mapRowContains(
  ...
  static async createBroadcast(uid, user, payload = {}) {
    ...
    await Promise.all(
      normalizeRoleList(row.targetRoles).map((role) =>
  ...
  static async createPromotion(uid, user, payload = {}) {
    await this._assertAdmin(uid, user);

    const title = safeString(payload.title);
    const description = safeString(payload.description || payload.message);
    const imageUrl = safeString(payload.imageUrl);
    const targetValue = safeString(payload.targetValue);
    if (title.length < 3) {
      const error = new Error("title is required (min 3 chars)");
      error.status = 400;
      throw error;
    ...
    if (description.length < 3) {
      const error = new Error("description is required (min 3 chars)");
      error.status = 400;
      throw error;
    }
    if (!imageUrl) {
    ...
    const targetType = normalizePromotionTargetType(payload.targetType);
    const placement = normalizePromotionPlacement(payload.placement);
    const badgeLabel = safeString(payload.badgeLabel) || "Featured";
    const ctaLabel = safeString(payload.ctaLabel) || "Explore";
    const query = safeString(payload.query);
    const category = safeString(payload.category);
    const city = safeString(payload.city);
    const targetRoles = normalizeRoleList(payload.targetRoles, ["finder"]);
    const sortOrder = Math.max(0, Math.floor(toNumber(payload.sortOrder, 0)));
    const active = parseBoolean(payload.active, true);
    const startAt = toIsoOrNull(payload.startAt);
    const endAt = toIsoOrNull(payload.endAt);
    if (payload.startAt !== undefined && !startAt) {
    ...
    const now = admin.firestore.FieldValue.serverTimestamp();
    const ref = db.collection("adminPromotions").doc();
    await ref.set({
      id: ref.id,
      placement,
      badgeLabel,
    ...
    const row = created.data() || {};
    await Promise.all(
      normalizeRoleList(row.targetRoles, ["finder"]).map((role) =>
        PushService.sendToRoles({
          roles: [role],
          title,
          body: description,
    ...
    return {
      data: {
        id: created.id,
        placement: normalizePromotionPlacement(row.placement),
        badgeLabel: safeString(row.badgeLabel),
        title: safeString(row.title),
        description: safeString(row.description),
        imageUrl: safeString(row.imageUrl),
        ctaLabel: safeString(row.ctaLabel),
        targetType: normalizePromotionTargetType(row.targetType),
        targetValue: safeString(row.targetValue),
        query: safeString(row.query),
        category: safeString(row.category),
  ...
  static async updatePromotionActive(uid, user, promotionId, payload = {}) {
    await this._assertAdmin(uid, user);
    const active = parseBoolean(payload.active, true);
    const ref = db.collection("adminPromotions").doc(promotionId);
  ...
  static async updateUserStatus(uid, user, userId, payload = {}) {
    ...
    const active = parseBoolean(payload.active, true);
    assertReasonIfRequired(reason, adminUserStatusNeedsReason(active));
  ...
  static async updateProviderKycStatus(uid, user, providerId, payload = {}) {
    await this._assertAdmin(uid, user);
    const reason = safeString(payload.reason || payload.note);
    const status = normalizeStatus(payload.status, "unverified");
    assertReasonIfRequired(reason, adminProviderKycNeedsReason(status));
    if (!ALLOWED_KYC_STATUSES.includes(status)) {
      const error = new Error("invalid provider KYC status");
      error.status = 400;
      throw error;
    }
    ...
    const row = snap.data() || {};
    const previousState = {
      kycStatus: providerKycStatus(row),
    ...
    const undo = await this._createUndoAction({
      adminUid: uid,
      actionType: "provider_kyc",
      targetLabel: providerId,
      docPath: `providers/${providerId}`,
      previousState,
      nextState: { kycStatus: status, verified: nextVerified },
      reason,
    });
    return {
      data: { id: providerId, status, verified: nextVerified, reason, ...undo },
    };
  }
  ...
  static async updateOrderStatus(uid, user, orderId, payload = {}) {
    ...
    assertReasonIfRequired(reason, adminOrderStatusNeedsReason(status));
  ...
  static async updatePostStatus(uid, user, source, postId, payload = {}) {
    ...
    const status = normalizeStatus(payload.status);
    assertReasonIfRequired(reason, adminPostStatusNeedsReason(status));
  ...
  static async updateTicketStatus(uid, user, userUid, ticketId, payload = {}) {
    ...
    const reason = safeString(payload.reason || payload.note);
    const status = normalizeStatus(payload.status);
    assertReasonIfRequired(reason, adminTicketStatusNeedsReason(status));
  ...
  static async updateServiceActive(uid, user, serviceId, payload = {}) {
    ...
    const reason = safeString(payload.reason || payload.note);
    const active = parseBoolean(payload.active, true);
    assertReasonIfRequired(reason, adminServiceActiveNeedsReason(active));
