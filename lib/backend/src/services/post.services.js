...
/** Batch-enrich posts with subscription tier from providers collection */
async function enrichWithSubscriptionTier(items) {
  ...
  for (const chunk of chunks) {
    ...
    snaps.forEach((snap) => {
      if (snap.exists) {
        ...
        tierMap[snap.id] = (data.subscription?.tier || "basic").toString();
      }
    });
  }
  ...
  return items.map((item) => {
    const tier = tierMap[item.providerUid] || "basic";
    if (tier !== "basic") {
  ...
  static async getProviderOffers(pagination) {
    ...
    if (canUseCursor) {
      ...
      const items = docs
        ...
        .filter((item) => item.status === "open");
      const enriched = await enrichWithSubscriptionTier(items);
