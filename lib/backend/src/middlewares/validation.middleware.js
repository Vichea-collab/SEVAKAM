...
function normalizeServiceList(body = {}) {
  const values = [];
  if (Array.isArray(body.services)) {
    body.services.forEach((entry) => {
      if (!isNonEmptyString(entry)) return;
      values.push(entry.trim());
    });
  }
  if (isNonEmptyString(body.service)) {
    values.push(body.service.trim());
  }
...
export const validateUserSettingsUpdate = withValidation((req) => {
  const body = req.body || {};
  if (!isObject(body)) return "Invalid body";
  if (!hasAnyField(body, ["notifications"])) {
    return "notifications is required";
  }
...
export const validateHelpTicketCreate = withValidation((req) => {
  ...
  if (!isNonEmptyString(body.subcategory)) return "subcategory is required";
  return null;
});
...
export const validateUserAddressUpdate = withValidation((req) => {
  const body = req.body || {};
  if (!isObject(body)) return "Invalid body";
  if (!hasAnyField(body, ["label", "street", "city", "mapLink", "isDefault"])) {
    return "At least one address field is required";
  }
  if (body.label !== undefined && !isNonEmptyString(body.label)) {
    return "label must be a non-empty string";
  }
  if (body.street !== undefined && !isNonEmptyString(body.street)) {
...
export const validateProviderProfileUpdate = withValidation((req) => {
  ...
  if (body.blockedDates !== undefined) {
    if (!Array.isArray(body.blockedDates)) {
      return "blockedDates must be an array";
    }
    const hasInvalidDate = body.blockedDates.some((value) => {
      if (typeof value !== "string" || value.trim().length === 0) return true;
      const parsed = Date.parse(value);
      return Number.isNaN(parsed);
    });
    if (hasInvalidDate) {
      return "blockedDates must contain valid date strings";
    }
  }
  if (body.kycStatus !== undefined) {
    const kycStatus = body.kycStatus.toString().trim().toLowerCase();
    if (!["pending", "unverified"].includes(kycStatus)) {
      return "kycStatus must be pending or unverified";
    }
  }
  if (body.kycIdFrontUrl !== undefined && !isNonEmptyString(body.kycIdFrontUrl)) {
    return "kycIdFrontUrl must be a non-empty string";
  }
  if (body.kycIdBackUrl !== undefined && !isNonEmptyString(body.kycIdBackUrl)) {
    return "kycIdBackUrl must be a non-empty string";
  }
  return null;
});
...
export const validateFinderPostCreate = withValidation((req) => {
  ...
  if (!isObject(body)) return "Invalid body";
  if (!isNonEmptyString(body.category)) return "category is required";
  const services = normalizeServiceList(body);
  if (services.length == 0) {
    return "service or services[] is required";
  }
  if (body.services !== undefined && !Array.isArray(body.services)) {
    return "services must be an array of non-empty strings";
  }
  if (body.location !== undefined && !isNonEmptyString(body.location)) {
    return "location must be a non-empty string";
  }
...
export const validateFinderPostUpdate = withValidation((req) => {
  const body = req.body || {};
  if (!isObject(body)) return "Invalid body";
  if (!hasAnyField(body, ["category", "service", "services", "location", "message", "preferredDate"])) {
    return "At least one post field is required";
  }
  ...
  if (body.service !== undefined && !isNonEmptyString(body.service)) {
    return "service must be a non-empty string";
  }
  if (body.service !== undefined || body.services !== undefined) {
    const services = normalizeServiceList(body);
    if (services.length == 0) {
      return "service or services[] is required";
    }
  ...
  if (body.message !== undefined && !isNonEmptyString(body.message)) {
...
export const validateProviderPostUpdate = withValidation((req) => {
  const body = req.body || {};
  if (!isObject(body)) return "Invalid body";
  if (
    !hasAnyField(body, [
  ...
  if (body.area !== undefined && !isNonEmptyString(body.area)) {
    return "area must be a non-empty string";
  }
  if (body.details !== undefined && !isNonEmptyString(body.details)) {
    return "details must be a non-empty string";
  }
  if (body.availableNow !== undefined && typeof body.availableNow !== "boolean") {
    return "availableNow must be a boolean";
  }
...
export const validateAdminPromotionCreate = withValidation((req) => {
  const body = req.body || {};
  if (!isObject(body)) return "Invalid body";
  if (!isNonEmptyString(body.title)) return "title is required";
  if (!isNonEmptyString(body.description ?? body.message)) {
    return "description is required";
  }
  if (!isNonEmptyString(body.imageUrl)) return "imageUrl is required";
  if (!isNonEmptyString(body.targetValue)) return "targetValue is required";

  if (body.targetType !== undefined) {
    const targetType = body.targetType.toString().trim().toLowerCase();
    if (!["provider", "service", "category", "search", "post", "page"].includes(targetType)) {
      return "targetType is invalid";
    }
  }

  if (body.placement !== undefined) {
    const placement = body.placement.toString().trim().toLowerCase();
    if (!["finder_home"].includes(placement)) {
      return "placement is invalid";
    }
  }
...
export const validateAdminPromotionActiveUpdate = withValidation((req) => {
  const body = req.body || {};
  if (!isObject(body)) return "Invalid body";
  if (typeof body.active !== "boolean") {
    return "active is required";
...
export const validateOrderReviewSubmit = withValidation((req) => {
  const body = req.body || {};
  if (!isObject(body)) return "Invalid body";
