...
class OrderController {
  ...
  static async getFinderOrders(req, res) {
    try {
      ...
      const result = await OrderService.getFinderOrders(
        req.user.uid,
        pagination,
        req.query || {},
      );
  ...
  static async submitFinderReview(req, res) {
    try {
      const orderId = (req.params.id || "").toString().trim();
      if (!orderId) return badRequest(res, "order id is required");
      const result = await OrderService.submitFinderReview(
        req.user.uid,
        req.user,
        orderId,
        req.body || {},
      );
      return ok(res, result.data, "Order review submitted successfully");
    } catch (error) {
      if (error.status === 404) {
        return notFound(res, error.message);
      }
      ...
      return internalServerError(
        res,
        error?.message || "Failed to submit order review",
      );
    }
  }
  ...
  static async getProviderReviews(req, res) {
    try {
      const providerUid = (req.params.providerUid || "").toString().trim();
      if (!providerUid) return badRequest(res, "provider uid is required");
      const result = await OrderService.getProviderReviews(providerUid, {
        limit: req.query?.limit,
      });
      return ok(res, result.data, "Provider reviews retrieved successfully");
    } catch (error) {
      if (error.status === 404) {
        return notFound(res, error.message);
      }
      ...
      return internalServerError(
        res,
        error?.message || "Failed to retrieve provider reviews",
      );
    }
  }
}
