...
function handleError(res, error, fallbackMessage) {
  ...
  if (error?.status === 429) {
    return tooManyRequests(res, error.message || "read budget exceeded");
  }
...
class AdminController {
  static async verifyAccess(req, res) {
    try {
      const result = await AdminService.verifyAccess(req.user.uid, req.user);
      return ok(res, result.data, "Admin access verified");
    } catch (error) {
      return handleError(res, error, "Failed to verify admin access");
    }
  }
  ...
  static async getPromotions(req, res) {
    try {
      ...
      return okPaginated(
        ...
        result.data,
        result.pagination,
        "Admin promotions retrieved successfully",
      );
    } catch (error) {
      return handleError(res, error, "Failed to retrieve admin promotions");
    }
  }
  ...
  static async createPromotion(req, res) {
    try {
      ...
      return ok(res, result.data, "Promotion created successfully");
    } catch (error) {
      return handleError(res, error, "Failed to create promotion");
    }
  }

  static async updatePromotionActive(req, res) {
    try {
      const result = await AdminService.updatePromotionActive(
        req.user.uid,
        req.user,
        req.params.id,
        req.body || {},
      );
      return ok(res, result.data, "Promotion state updated successfully");
    } catch (error) {
      return handleError(res, error, "Failed to update promotion state");
    }
  }
  ...
  static async updateProviderKycStatus(req, res) {
    try {
      const result = await AdminService.updateProviderKycStatus(
        req.user.uid,
        req.user,
        req.params.id,
        req.body || {},
      );
      return ok(res, result.data, "Provider KYC updated successfully");
    } catch (error) {
      return handleError(res, error, "Failed to update provider KYC");
    }
  }
