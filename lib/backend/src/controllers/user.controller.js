...
class UserController {
    ...
    static async getPromotions(req, res) {
        try {
            const result = await UserService.getPromotions(req.user.uid, req.query || {});
            return ok(res, result.data, "Promotions retrieved successfully");
        } catch (err) {
            return internalServerError(res, "Failed to retrieve promotions");
        }
    }

    static async getNotificationReadState(req, res) {
        try {
            const result = await UserService.getNotificationReadState(req.user.uid);
            return ok(res, result.data, "Notification read state retrieved successfully");
        } catch (err) {
            return internalServerError(res, "Failed to retrieve notification read state");
        }
    }

    static async updateNotificationReadState(req, res) {
        try {
            const body = req.body || {};
            if (body.readKeys !== undefined && !Array.isArray(body.readKeys)) {
                return badRequest(res, "readKeys must be an array");
            }
            if (body.clearedKeys !== undefined && !Array.isArray(body.clearedKeys)) {
                return badRequest(res, "clearedKeys must be an array");
            }
            const result = await UserService.updateNotificationReadState(
                req.user.uid,
                body,
            );
            return ok(res, result.data, "Notification read state updated successfully");
        } catch (err) {
            return internalServerError(res, "Failed to update notification read state");
        }
    }

    static async registerPushToken(req, res) {
        try {
            const result = await UserService.registerPushToken(req.user.uid, req.body || {});
            return ok(res, result.data, "Push token registered successfully");
        } catch (err) {
            if (err.status === 400) {
                return badRequest(res, err.message);
            }
            return internalServerError(res, "Failed to register push token");
        }
    }

    static async unregisterPushToken(req, res) {
        try {
            const result = await UserService.unregisterPushToken(req.user.uid, req.body || {});
            return ok(res, result.data, "Push token unregistered successfully");
        } catch (err) {
            if (err.status === 400) {
                return badRequest(res, err.message);
            }
            return internalServerError(res, "Failed to unregister push token");
        }
    }
    ...
    static async createHelpTicket(req, res) {
        try {
            const { ticketType, category, subcategory } = req.body || {};
            if (!category || !subcategory) {
                return badRequest(res, "category and subcategory are required");
            }
            const result = await UserService.createHelpTicket(req.user.uid, {
                ticketType,
                category,
                subcategory,
            });
